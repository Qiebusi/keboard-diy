...
const Render = (() => {
  ...
  /* 键帽几何（返回顶面矩形） */
  function capGeom(key, U) {
    const m = U * 0.045;                       // 键间隙
    const x0 = key.x * U + m;
    const y0 = key.y * U + m;
    const w = key.w * U - 2 * m;
    const h = key.h * U - 2 * m;
    const ti = U * 0.125;                      // 顶面内缩（1u 顶面 ≈ 0.66u）
    const tw = Math.max(w - 2 * ti, U * 0.2);
    const th = Math.max(h - 2 * ti, U * 0.2);
    return {
      x: x0, y: y0, w, h,
      tx: x0 + (w - tw) / 2,
      ty: y0 + (h - th) / 2,
   ...
   * @param o {U, selected, hover, getImg, exportMode}
   */
  function drawKey(ctx, key, d, o) {
    const U = o.U;
    const g = capGeom(key, U);
